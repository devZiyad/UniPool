    ...
    try {
      ...
      for (final booking in activeBookings) {
        try {
          ...
          print('Error loading ride ${booking.rideId} for booking ${booking.id}: $e');
        }
      }

      // Filter out bookings where the ride is cancelled
      final validBookings = activeBookings.where((booking) {
        final ride = rideDetailsMap[booking.rideId];
        if (ride == null) {
          // If we couldn't load the ride, include it (better to show than hide)
          return true;
        }
        // Exclude if ride is cancelled (case-insensitive)
        return ride.status.toUpperCase() != 'CANCELLED';
      }).toList();
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            Text(
                                              booking.pickupLocationLabel ?? 'Pickup location',
                                              style: const TextStyle(
                                                fontSize: 14,
                                                fontWeight: FontWeight.w500,
                                              ),
                                            ...
                                            const SizedBox(height: 8),
                                            Text(
                                              booking.dropoffLocationLabel ?? 'Dropoff location',
                                              style: const TextStyle(
                                                fontSize: 14,
                                                fontWeight: FontWeight.w500,
                                              ),
                                  ...
                                  if (booking.pickupTimeStart != null || booking.pickupTimeEnd != null) ...[
                                    const SizedBox(height: 8),
                                    Text(
                                      _formatTimeRange(
